...
@Component({
  ...
  template: `
    <button
      ...
      class="fixed right-4 bottom-4 z-50 flex h-14 w-14 items-center justify-center rounded-full border border-white/60 bg-white/40 shadow-xl shadow-stone-200/50 backdrop-blur-md transition-transform duration-300 hover:scale-110 active:scale-95"
    >
      ...
      @if (reproduciendo()) {
        <span class="absolute inset-0 flex items-center justify-center gap-[3px]" aria-hidden="true">
          @for (retraso of ondas; track $index) {
            <span
              class="animate-sound-wave h-6 w-[3px] rounded-full bg-rose-300/70"
      ...
      <i
        class="pi relative text-lg text-rose-600"
