...
@Component({
  ...
  template: `
    <div
      class="mx-auto max-w-md rounded-3xl border border-white/60 bg-white/40 p-8 shadow-xl shadow-stone-200/50 backdrop-blur-md"
    >
      <div class="text-center">
        <i class="pi pi-envelope mb-4 text-3xl text-rose-500" aria-hidden="true"></i>
        <h3 class="font-serif text-2xl text-stone-800">Confirma tu asistencia</h3>
      ...
      <form class="mt-6 flex flex-col gap-4" (submit)="$event.preventDefault(); confirmar()">
        <div class="flex flex-col gap-1">
          <label for="nombre" class="text-xs font-medium tracking-wide text-stone-600 uppercase">
            Tu nombre completo
          </label>
          <input
            pInputText
        ...
        <div class="flex flex-col gap-1">
          <label for="pases" class="text-xs font-medium tracking-wide text-stone-600 uppercase">
            Número de lugares
          </label>
          <p-select
            inputId="pases"
      ...
      <div class="my-6 flex items-center gap-3">
        ...
        <span class="text-xs font-light text-stone-400">o</span>
        <span class="h-px flex-1 bg-stone-200"></span>
      </div>
